...
client.on("message", async message => {
    ...
    if (comando === "registrar"){
        ...
        m.edit(embed)
        message.channel.send(`${message.member} vá ao canal #função e reaja de acordo com o que você joga.`)
    }
    ...
    if (comando === "teste"){
        const embed = new Discord.RichEmbed()
            .setColor(0x0F0FF0)
            .setDescription(`:wave: Bem vindo(a) ${message.member} :wave:`)
            .addBlankField()
            .addField(' Membro da guild :punch: ', ' No canal ` #registro ` digite: ``` !registrar SeuNickNoJogo ```')
            .addField(' Convidado :handshake: ', ' No canal ` #registro ` digite: ``` !convidado ```')
        message.channel.send(embed)
        message.channel.send(`${message.member} vá ao canal ${580482165792309273} para se registrar.`)
    }
})
